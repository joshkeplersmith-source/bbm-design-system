...
**Ghosted background text:** Large Playfair Display text at 8–12% opacity behind section content. N-100 sections only. Once per page maximum.
...
- Use Work Sans 300 for all body copy; Work Sans 600 for bold emphasis sparingly
...
1. What color is the zone the logo sits in?
   ...
   - Light background, brand-forward → **Bu variant** (blue BU-700)
...
**Default surface:** N-100 with texture is the default for all standalone deliverables — posters, emails, landing pages, social, documents. BU-700 is for intentional contrast within a layout, never the default starting point. When in doubt, build light.
...
**Layout DO:** Textures 90° filling N-100 sections · Playfair Display ghosted text on N-100 · L-bracket gold frame for featured content · NV-700/500 callout lines
...
Every piece of content must be traceable back to this statement.
...
**Primary:** Woman, early-to-mid 40s, professional, high-achieving. Notices a gap between how she wants to feel and how she actually feels. Done with providers who address only half the picture. Trust is the real barrier — not price.

**Secondary:** Men 40s–60s — TRT, peptides, metabolic wellness. Do not alienate at the brand level.

**Not for:** Budget-driven one-off shoppers · patients wanting pure aesthetics with no relationship · clinical-coldness seekers · trend-driven influencer-aesthetic seekers
...
**Not:** a luxury destination · a clinical optimization lab · a trend-driven med spa · an anti-aging brand · a beauty brand

**Visual anti-brief:** No white-box photography · no bright saturated color · no measuring tape or scale · no stock wellness poses · no legacy Poppins all-caps system · no hard-bordered before/after blocks
...
|---|---|---|---|---|
| HTML email templates | GHL | Full — R2 URLs resolve | Full | Google Fonts via `<link>` |
| Web pages | Webflow / WordPress | Full — R2 URLs resolve | Full | Use CSS tokens from Section 1.14 |
| Print collateral (poster/flyer) | HTML → Chrome browser print | Full — R2 URLs resolve in browser | Full | Build HTML with Google Fonts via `<link>`. Chrome → File → Print → Save as PDF. Background graphics: on, margins: none. |
...
Colors, typography, spacing, voice, and layout rules are all still available. Build with those.

**Textures:** Omit silently. No mention, no flag, no request. A page without textures is still fully on-brand.
...
Wait for the upload. Do not substitute a placeholder.
...
```
☐ Confirm BB-BRAND-TEAM.md version is current
...
Before building any visual output, confirm the environment can render EB Garamond, Playfair Display, and Work Sans. If it cannot, route to HTML with Google Fonts via `<link>` — do not build in a degraded environment.

Never produce output where brand fonts are not rendering correctly. Once built as HTML, guide the user through Chrome browser-print to get a shareable PDF. The user does not need to know why — just get them to a correctly branded file.
...
- **Neither** → pause and give the user a specific image brief using the prompt in Section 6
...
- Apply voice and vocabulary rules from Section 2.5 and 2.7 to all copy.
